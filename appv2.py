...
with tab_insights:
    ...
    # OVERALL INSIGHTS
    # =====================================================
    if insight_scope == "Overall (All Companies)":

        overall_df = overall_series.reset_index()
        overall_df.columns = ["Reason", "Percent"]
        overall_df = overall_df.sort_values("Percent", ascending=False)

        top_reason = overall_df.iloc[0]
        bottom_reason = overall_df.iloc[-1]

        paragraph = (
            f"Across all companies, the most frequently cited driver of headcount change in 2025 "
            f"is {top_reason['Reason']} ({top_reason['Percent']:.0f}%). "
            f"At the other end of the spectrum, {bottom_reason['Reason']} "
            f"is among the least cited factors ({bottom_reason['Percent']:.0f}%), "
            f"suggesting it is either more situational or less broadly felt."
        )

        st.write(paragraph)

        st.markdown("**Top 5 Drivers (Overall):**")
        for _, r in overall_df.head(5).iterrows():
            st.write(f"- {r['Reason']} — {r['Percent']:.0f}%")

    # =====================================================
    # CATEGORY INSIGHTS (NO AVERAGING)
    # =====================================================
    else:

        category = insight_scope
        cat_df = df_working[df_working["Category"] == category].copy()

        # Melt
        long_cat = cat_df.melt(
            id_vars=["Category", "Subcategory"],
            value_vars=reason_cols,
            var_name="Reason",
            value_name="Percent"
        ...
        long_cat["Percent"] = pd.to_numeric(long_cat["Percent"], errors="coerce")
        long_cat = long_cat.dropna(subset=["Percent"])

        # Add Overall benchmark
        overall_map = overall_series.to_dict()
        long_cat["OverallPercent"] = long_cat["Reason"].map(overall_map)
        long_cat["Delta"] = long_cat["Percent"] - long_cat["OverallPercent"]

        # -----------------------------------------------------
        # Largest positive delta (subcat vs overall)
        # -----------------------------------------------------
        most_above = long_cat.sort_values("Delta", ascending=False).iloc[0]

        # Largest negative delta
        most_below = long_cat.sort_values("Delta", ascending=True).iloc[0]

        # Most polarizing driver within category
        spread = (
            long_cat.groupby("Reason")["Percent"]
            .agg(lambda x: x.max() - x.min())
            .sort_values(ascending=False)
        )

        most_polarizing_reason = spread.index[0]
        max_range = spread.iloc[0]

        # Top overall driver (anchor context)
        overall_df = overall_series.reset_index()
        overall_df.columns = ["Reason", "Percent"]
        overall_df = overall_df.sort_values("Percent", ascending=False)
        ...
        paragraph = (
            f"While the overall population identifies {top_overall['Reason']} "
            f"as the dominant driver of headcount change ({top_overall['Percent']:.0f}%), "
            f"within {category}, the strongest divergence appears in "
            f"{most_above['Reason']}, where {most_above['Subcategory']} "
            f"over-indexes the overall benchmark by {most_above['Delta']:.0f} points. "
            f"Conversely, {most_below['Subcategory']} under-indexes most sharply on "
            f"{most_below['Reason']} ({most_below['Delta']:.0f} points below overall). "
            f"The greatest internal variation within the category occurs around "
            f"{most_polarizing_reason} (a {max_range:.0f}-point spread across subcategories), "
            f"suggesting uneven exposure to that driver."
        )

        st.markdown(paragraph)

        st.markdown("**Most Over-Indexing Subcategory:**")
        st.write(
            f"- {most_above['Subcategory']} — {most_above['Reason']} "
            f"(+{most_above['Delta']:.0f} pts vs Overall)"
        )

        st.markdown("**Most Under-Indexing Subcategory:**")
        st.write(
            f"- {most_below['Subcategory']} — {most_below['Reason']} "
            f"({most_below['Delta']:.0f} pts vs Overall)"
        )

        st.markdown("**Most Polarizing Driver Within Category:**")
        st.write(
            f"- {most_polarizing_reason} — {max_range:.0f}-point spread"
        )
